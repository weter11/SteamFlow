...
pub fn detect_runner_components(
    ...
    let mut dxvk = dxvk;
    let mut d7vk = d7vk;
    let mut vkd3d_proton = vkd3d_proton;
    let mut vkd3d = vkd3d;
    let mut nvapi = nvapi;
...
fn detect_dxvk_nvapi(root: &Path, prefix: Option<&Path>) -> Option<ComponentInfo> {
    ...
    let mut required = vec!["nvapi64.dll", "nvofapi64.dll"];
    let mut alt_required = vec!["nvapi64.dll"];
...
/// Known keys: WINE_COMMIT, DXVK_VERSION, D7VK_VERSION,
/// VKD3D_VERSION, VKD3D_PROTON_VERSION, DXVK_NVAPI_VERSION,
/// WINE_MONO_VERSION, WINE_GECKO_VERSION, BUILD_DATE.
