...
";
    ...
    pub(crate) fn spawn_game_process(
        ...
        match launch_info.target {
            ...
            LaunchTarget::WindowsProton => {
                use crate::infra::runners::{Runner, WineTkgRunner, LaunchContext};
                ...
                let ctx = LaunchContext {
                    ...
                    user_config: user_config.cloned(),
                    proton_path: proton_path.map(|s| s.to_string()),
                };

                runner.prepare_prefix(&ctx)?;
                let spec = runner.build_command(&ctx)?;
                runner.launch(&spec)
            }
        }
    }
