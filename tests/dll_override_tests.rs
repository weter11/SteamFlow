...
fn test_build_dll_overrides_baseline() {
    ...
    let overrides = build_dll_overrides(false, false, false, false, None);
...
fn test_build_dll_overrides_dxvk_active() {
    let overrides = build_dll_overrides(true, false, false, true, None);
...
fn test_build_dll_overrides_vkd3d_active() {
    let overrides = build_dll_overrides(false, true, false, true, None);
...
fn test_build_dll_overrides_local_dll_skip() {
    ...
    let overrides = build_dll_overrides(true, false, false, true, Some(tmp.path()));
